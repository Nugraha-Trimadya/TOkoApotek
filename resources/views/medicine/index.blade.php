    ...
    @section('content')
        <div class="container">
            ...
            <a href="{{ route('obat.admin.export') }}" class="btn btn-success">Export Excel</a>
                        ...
                        @foreach ($medicines as $index => $item)
                            <tr>
                                <td class="text-center">
                                    {{ ($medicines->currentpage() - 1) * $medicines->perPage() + ($index + 1) }}</td>
                                <td class="text-center">{{ $item['name'] }}</td>
                                <td class="text-center">{{ $item['type'] }}</td>
                                <td class="text-center">Rp. {{ number_format($item['price'], 0, ',', ',') }}</td>
                                <td class="{{ $item['stock'] <= 3 ? 'bg-danger text-white' : '' }} text-center"
    ...
    @push('script')
        ...
        <script>
            ...
            function showModalStock(id, stock) {
                ...
                let url = "{{ route('obat.edit_stock', ':id') }}";
